...
def rotate_whole_scene_by_track_index(polylines, ori_obj_trajs_full, all_mtr_predicted_traj, all_llm_augmented_mtr_predicted_traj, track_index_to_predict):
    ...
    for predicted_traj in all_mtr_predicted_traj:
    ...
    for predicted_traj in all_llm_augmented_mtr_predicted_traj:
        predicted_traj["pred_trajs"][:, :, :2] -= local_obj_point[:2]
        predicted_traj["gt_trajs"][:, :2] -= local_obj_point[:2]
    ...
    return polylines, ori_obj_trajs_full, all_mtr_predicted_traj, all_llm_augmented_mtr_predicted_traj

def vis_frame(mtr_all_predicted_trajs, llm_augmented_mtr_all_predicted_trajs, ori_data_path, output_path):
    scenario_id = mtr_all_predicted_trajs[0]["scenario_id"]
    ...
    for plot_index, track_index_to_predict in enumerate(ori_tracks_to_predict_track_index):
        ...
        polylines_rotated, ori_obj_trajs_full_rotated, all_mtr_predicted_traj_rotated, all_llm_augmented_mtr_predicted_traj_rotated = rotate_whole_scene_by_track_index(copy.deepcopy(polylines), copy.deepcopy(ori_obj_trajs_full), copy.deepcopy(mtr_all_predicted_trajs), copy.deepcopy(llm_augmented_mtr_all_predicted_trajs), track_index_to_predict)

        # fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(64, 32))
        fig = plt.figure(figsize=(80, 32), dpi=200, facecolor='white')
        ...
        DrawCar(scenario_id, ori_obj_trajs_full_rotated, ori_sdc_track_index, ori_tracks_to_predict_track_index, ori_obj_ids, ori_obj_types, ax1)
        DrawPictures(ori_obj_trajs_full_rotated, all_mtr_predicted_traj_rotated, 8, ori_sdc_track_index, ori_tracks_to_predict_track_index, ori_obj_ids, ax1)
        ...
        DrawCar(scenario_id, ori_obj_trajs_full_rotated, ori_sdc_track_index, ori_tracks_to_predict_track_index, ori_obj_ids, ori_obj_types, ax2)
        DrawPictures(ori_obj_trajs_full_rotated, all_llm_augmented_mtr_predicted_traj_rotated, 8, ori_sdc_track_index, ori_tracks_to_predict_track_index, ori_obj_ids, ax2)
        ...
        fig.savefig(f"{output_path}/{scenario_id}_{track_index_to_predict}_multi_agent_compare.png")
        plt.close()
...
def main():
    ...
    for s_id in tqdm(vis_dict.keys()):
        ...
        mtr_all_predicted_trajs = mtr_data[mtr_index_list[0]]
        ...
        llm_augmented_mtr_all_predicted_trajs = llm_augmented_mtr_data[llm_augmented_mtr_index_list[0]]        
        
        vis_frame(mtr_all_predicted_trajs, llm_augmented_mtr_all_predicted_trajs, ori_data_path, output_path)
