...
def plt_crosswalks(crosswalks, polylines):
    for crosswalk in crosswalks:
        ...
        plt.fill(cross_polylines[:, 0], cross_polylines[:, 1], color='#E0D9D8', hatch='//', edgecolor='#494949', alpha=0.5, zorder=3) # #ABABAB

def plt_lanes(lanes, polylines, lane_head_width=1, lane_head_height=1.4):
...
def plt_road_lines(road_lines, polylines):
    for line in road_lines:
        ...
        #     plt.plot(line_polylines[:, 0], line_polylines[:, 1], color='yellow', alpha=1, zorder=2, linewidth=4, linestyle='dashed')
        # elif line_type == 'TYPE_SOLID_SINGLE_YELLOW' or line_type == 'TYPE_SOLID_DOUBLE_YELLOW':
        #     plt.plot(line_polylines[:, 0], line_polylines[:, 1], color='yellow', alpha=1, zorder=2, linewidth=4)
        # else:  # TYPE_UNKNOWN, TYPE_BROKEN_SINGLE_WHITE, TYPE_PASSING_DOUBLE_YELLOW
        #     plt.plot(line_polylines[:, 0], line_polylines[:, 1], color='white', alpha=1, zorder=2, linewidth=4, linestyle='dashed')

def DrawMap(polylines, ori_map_infos):
...
def DrawCar(scenario_id, ori_obj_trajs_full, ori_sdc_track_index, track_index_to_predict, ori_obj_ids, ori_obj_types):
    st, ed = (0, 11)
    for i in range(ori_obj_trajs_full.shape[0]):
        # [cx, cy, cz, dx, dy, dz, heading, vel_x, vel_y, valid]
        obj_traj = ori_obj_trajs_full[i][st:ed]
        ...
        # plt.text(position_x[-1], position_y[-1], i, color='black', zorder=20, fontweight="bold")
        ...
        plt.plot([x1[-1], x2[-1], x3[-1], x4[-1], x1[-1]], [y1[-1], y2[-1], y3[-1], y4[-1], y1[-1]], color=color_bbox, zorder=30, alpha=0.7)
        plt.fill([x1[-1], x2[-1], x3[-1], x4[-1]], [y1[-1], y2[-1], y3[-1], y4[-1]], color=color, zorder=30, alpha=0.7)

        # plt.fill([(x1[-1] + x2[-1]) / 2, x6[-1], x5[-1],], [(y1[-1] + y2[-1]) / 2, y6[-1], y5[-1],], color="black", zorder=10)
...
def DrawThresholdBox(obj_traj, obj_valid, t, color):
    ...
    if obj_valid[idx]:
        ...
        plt.plot([bbox_x_1, bbox_x_2, bbox_x_3, bbox_x_4, bbox_x_1], [bbox_y_1, bbox_y_2, bbox_y_3, bbox_y_4, bbox_y_1], color=color, zorder=100, alpha=1)

def DrawGroundTruth(ori_obj_trajs_full, timeStamp, ori_sdc_track_index, track_index_to_predict, ori_obj_ids):
    ...
    for i in range(ori_obj_trajs_full.shape[0]):
        ...
        if i == track_index_to_predict:
            color = interested_agent_color
            plt.plot(position_x, position_y, color=color, linewidth=10, zorder=20)
            plt.scatter(position_x[-1], position_y[-1], color=color, marker="*", s=2000, zorder=11)
        #     color = list(colorTable.values())[ori_obj_ids[i] % len(colorTable)]
        #     DrawThresholdBox(obj_traj, obj_valid, timeStamp, color)
        else:
            plt.plot(position_x, position_y, color=color, linewidth=3, zorder=20)
        # plt.scatter(position_x[-1], position_y[-1], color=color, marker="*", s=200, zorder=11)

        # if(timeStamp == 8 and obj_valid[-1] == False and i in ori_tracks_to_predict_track_index):
        #     plt.scatter(position_x[-1], position_y[-1], color=color, marker="*", s=200, zorder=11)

def DrawPredictTrajectories(predicted_traj, timeStamp):
    st, ed = GetBoundary(timeStamp)
    ...
    for i in range(len(color_list) - 1, -1, -1):
        ...
        plt.plot(pred_traj_x, pred_traj_y, color=color, linewidth=10, alpha=0.8, zorder=20)

        plt.scatter(pred_traj_x[-1], pred_traj_y[-1], color=color, marker=".", s=1000, zorder=20, alpha=1)

def DrawPictures(ori_obj_trajs_full, predicted_traj, timeStamp, ori_sdc_track_index, track_index_to_predict, ori_obj_ids):
    DrawGroundTruth(ori_obj_trajs_full, timeStamp, ori_sdc_track_index, track_index_to_predict, ori_obj_ids)
    DrawPredictTrajectories(predicted_traj, timeStamp)

def rotate_whole_scene_by_track_index(polylines, ori_obj_trajs_full, predicted_traj, track_index_to_predict):
    ...
    return polylines, ori_obj_trajs_full, predicted_traj
...
def vis_frame(all_predicted_trajs, ori_data_path, output_path):
    ...
    for plot_index, predicted_traj in enumerate(all_predicted_trajs):
        track_index_to_predict = ori_tracks_to_predict_track_index[plot_index]
        
        interested_agent_type = predicted_traj["object_type"]
        context_map_range = context_map_range_dict[interested_agent_type]
        
        polylines_rotated, ori_obj_trajs_full_rotated, predicted_traj_rotated = rotate_whole_scene_by_track_index(copy.deepcopy(polylines), copy.deepcopy(ori_obj_trajs_full), copy.deepcopy(predicted_traj), track_index_to_predict)
        ...
        plt.figure(figsize=(32, 32), dpi=300, facecolor="white")

        DrawMap(polylines_rotated, ori_map_infos)
        DrawCar(scenario_id, ori_obj_trajs_full_rotated, ori_sdc_track_index, track_index_to_predict, ori_obj_ids, ori_obj_types)
        DrawPictures(ori_obj_trajs_full_rotated, predicted_traj_rotated, 8, ori_sdc_track_index, track_index_to_predict, ori_obj_ids)
        
        # if(box_range != -1):
        plt.xlim([-context_map_range, context_map_range])
        plt.ylim([-context_map_range, context_map_range])
        # cmap = plt.colormaps["Reds"]
        # plt.colorbar(plt.cm.ScalarMappable(norm=Normalize(0, 1), cmap=cmap), label="possibility")
        plt.axis('off')
        plt.savefig(f"{output_path}/{scenario_id}_{track_index_to_predict}_single_agent.png", bbox_inches="tight")
        plt.close()
